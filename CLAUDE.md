...
uv run scripts/projects.py list [--json] | dashboard | digest [--days N] | notify [--dry-run] [--force] | scan-inbox [--dry-run] | new "Name"
...
- **Side effects are deliberate and constrained:**
  ...
  - eDoc inboxes are discovered at run time. An `EDOC_INBOX`/`EDOC_DIGEST_INBOX` name that is not on the ทางลัด tab is reported as `absent` (0, "likely no new documents"), not an error: an inbox with nothing new can drop off the tab (observed Sept 2026, unconfirmed). The listed names go in the note so a typo still shows, and an ambiguous name is still an error. This is only safe because the tab is read with a stability poll (its links load in stages); keep the poll in both scripts. Name matching ignores whitespace, because Thai titles appear both as "ผศ. ดร. X" and "ผศ.ดร.X". Two counts are reported per inbox: unread/ใหม่ (the actionable one) and the full ค้างรับ list, which the server caps at 500.
  ...
  - `projects.py` writes only `<vault>/<PM_FOLDER>/Dashboard.md`, `<PM_FOLDER>/User Manual.md` (from `templates/project-manager-manual.md`, rewritten only when missing or its `plugin_version` differs), new hub notes via `new`, and task lines `scan-inbox` appends to a hub's `## Suggested from inbox` section, all atomically (the vault is synced while it runs); it never edits other notes or any frontmatter, and skips `.obsidian/`, `.trash/`, `Confidential/`. `notify` sends only to `PM_NOTIFY_TO` and only when something is due — the user's "no email without approval" rule has a standing exception for exactly this digest. Its config lookup is a copy of `check_pending.py`'s; keep them aligned. `gmail-oauth` (the default) uses the bundled Desktop OAuth client `scripts/gmail_oauth_client.json` (Internal consent screen in the BUU Workspace; committed on purpose, since an installed app's secret isn't confidential) unless `PM_GMAIL_CLIENT` or `credentials.json` in the config dir overrides it. The token always lives in the config dir, never beside the bundled client in the version-pinned cache. `scan-inbox` needs `auth-gmail --scan` (adds gmail.readonly + calendar.readonly). It calls `claude -p` as a pure function (`--tools "" --strict-mcp-config --setting-sources ""`, a JSON schema), because email bodies are untrusted: Claude never writes, and the script validates every returned task before appending it. Keep it that way.
  - The `transcribe` skill (not `transcribe.py`, which stays vendored) `rclone move`s the audio and its `.md` transcript to `TRANSCRIBE_RCLONE_DEST` after the transcript has been checked and summarised. It checks both names for a clash first, doesn't move the `.md` if the audio move failed, and `none` opts out.
  - The plugin's only hook, `hooks/hooks.json` (`SessionStart`, `startup`), runs `projects.py manual --quiet`. Claude Code has no install/update hook, so this is how the manual refreshes after an update. It must stay silent and exit 0 when project tracking isn't set up, and it never creates the projects folder.
