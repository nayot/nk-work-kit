...
- **Side effects are deliberate and constrained:**
  ...
  - The `transcribe` skill (not `transcribe.py`, which stays vendored) `rclone move`s the audio to `TRANSCRIBE_RCLONE_DEST` after the transcript has been checked. It never moves the `.md`, checks for a name clash first, and `none` opts out.
